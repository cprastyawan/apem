...
<div id="auto">
	<table border="3" cellspacing="2" cellpadding="2"> 
      <tr> 
          ...
          <th> <font face="Arial">Tekanan</font> </th> 
      </tr>
	<?php
	include "koneksi.php";
	if(isset($_POST["ptanggal"])){
		echo $_POST["ptanggal"];
		$sql="select ID, pres, tanggal, waktu from bmp280Data where tanggal='" . $_POST["ptanggal"] . "' order by ID desc;";
		$result = $conn->query($sql);
		if(!$result) printf("ERROR: %s", $conn->error);
		else {
			if($result->num_rows>0){
				while($row=$result->fetch_assoc()){
					echo "<tr><td>" . $row["ID"]. "</td><td>" . $row["tanggal"] . "</td><td>" . $row["waktu"] . "</td><td>" . $row["pres"] . "</td></tr>";
			}
			echo "</table>";
