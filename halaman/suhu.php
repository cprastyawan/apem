...
<div id="auto">
	...
	if(isset($_POST["ptanggal"])){
		echo $_POST["ptanggal"];
		$sql="select ID, temp, tanggal, waktu from bmp280Data where tanggal='" . $_POST["ptanggal"] . "' order by ID desc;";
		$result = $conn->query($sql);
		if(!$result) printf("ERROR: %s", $conn->error);
		else {
			if($result->num_rows>0){
				while($row=$result->fetch_assoc()){
					echo "<tr><td>" . $row["ID"]. "</td><td>" . $row["tanggal"] . "</td><td>" . $row["waktu"] . "</td><td>" . $row["temp"] . "</td></tr>";
			}
			echo "</table>";
